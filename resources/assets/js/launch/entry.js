            ...
            $(".per").html('Por: '+data[0].nameUser);
                $.each(data, function (_indexInArray, valueOfElement) { 
                    if(typeof valueOfElement.file_launches_name !== 'undefined'){
                        ...
                        '<a href="'+SearaApp.baseURL+'/storage/images/'+valueOfElement.file_launches_name+'" class="thumbnail" data-lightbox="roadtrip" data-title="Conta: '+valueOfElement.entries_description+'"> <img src="'+SearaApp.baseURL+'/storage/images/'+valueOfElement.file_launches_name+'" '+'> </a>')
    ...
    _getFilesXhr = $.get(SearaApp.baseURL+'/info-launch/'+id+'/?desc='+desc+'&value='+value+'&date='+date+'&comp='+company_id, function (data, textStatus, jqXHR) {
        ...
        $.each(data, function (index, val) {
            ...
            if(val.hasOwnProperty('file_launches_name')) {
                ...
                '<td style="width: 20%">'+
                        '<a href="'+SearaApp.baseURL+'/storage/images/'+val.file_launches_name+'" data-lightbox="roadtrip" ' +
